...
static const char DeviceId[] = "beakerADevice";
static const char connectionString[] = "HostName=fgIoTHubTest.azure-devices.net;DeviceId=beakerADevice;SharedAccessKey=ivMVshMt0tRGo08W1es5XQ==";
static const char tipo[] = "BeakerA";
static int redLedPin = 13;
static int redLedState = LOW;
static int greenLedPin = 12;
            ...
            else
            {
                ...
                else
                {
                    ...
                    while (1)
                    {
                        ...
                        if (Curr_time_ms >= Prev_time_ms + 10000)
                        {
                            ...
                            LogInfo("Result: %s | %s | %d | %d | %d \r\n", myWeather->DeviceId, myWeather->tipo, myWeather->EventTime, myWeather->MTemperature, myWeather->Humidity);
                        
                            unsigned char* destination;
                            size_t destinationSize;
                            
                            if (SERIALIZE(&destination, &destinationSize, myWeather->DeviceId, myWeather->tipo, myWeather->EventTime, myWeather->MTemperature, myWeather->Humidity) != IOT_AGENT_OK)
                            {
